...
function get_position($typeid = 0, $sname = '', $surl = '', $ismobile = false, $delimiter = '&gt;&gt;')
{
    if ($delimiter == '') {
        //$delimiter = '&gt;&gt;';	//旧的分隔符
        $delimiter = '<img src="/public/home/default/img/introduce/jt_11.png" alt="" class="img-responsive jt" />';
    }
    $url = $ismobile ? U(MODULE_NAME . '/Index/index/') : C('CFG_WEBURL');
    $position = '<a class="js" href="' . $url . '">首页</a>';
    ...
    if (!empty($typeid)) {
        ...
        if (is_array($getParents)) {
            foreach ($getParents as $v) {
                $position .= $delimiter . '<a class="js" href="' . get_url($v) . '">' . $v['name'] . '</a>';
            }
        }
    }
    ...
    if (!empty($sname)) {
        if (empty($surl)) {
            ...
            $position .= $delimiter . '<a class="js" href="' . $surl . '">' . $sname . '</a>';
        }
    }
