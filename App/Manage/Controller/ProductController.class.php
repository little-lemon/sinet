...
class ProductController extends CommonContentController
{
    ...
    public function addPost()
    {
        ...
        if (empty($data['description'])) {
            $data['description'] = str2sub(strip_tags($data['content']), 500);
        }
        ...
        //技术优势的标题和内容设置
        $advtitle= I('adv_title', '');
        $advcontent = I('adv_content','');
        if(is_array($advtitle)){
            foreach($advtitle as $k => $v){
                $advtitle_arr[] = $v.'$$$'.'$$$';
                $advcontent_arr[] = $advcontent[$k].'$$$'.'$$$';
            }
        }
        $data['advantages'] = join('|||', $advtitle_arr);
        $data['advcontent'] = join('|||', $advcontent_arr);

        //典型应用标题和内容
        $apptitle = I('app_title', '');
        $appcontent= I('app_des', '');
        if(is_array($apptitle)){
            foreach($apptitle as $k => $v){
                if($v != '' ){
                    $apptitle_arr[] = $v.'$$$'.'$$$';
                    $appcontent_arr[] = $appcontent[$k].'$$$'.'$$$';
                }
            }
        }
        $data['apptitle'] = join('|||', $apptitle_arr);
        $data['appcontent'] = join('|||', $appcontent_arr);

        //典型应用图片
        $apppicurls_arr = array();
        $apppicurls= isset($data['app_pictureurls']) ? $data['app_pictureurls'] : '';
        if (is_array($apppicurls)) {
            foreach ($apppicurls as $k => $v) {
                $apppicurls_arr[] = $v . '$$$' . '$$$';
            }
        }
        $data['apppicurls'] = join('|||', $apppicurls_arr);
    ...
    public function edit()
    {
        ...
        unset($temparr2);
        $advtitle = array();
        if (!empty($vo['advantages'])) {
            $temparr = explode('|||', $vo['advantages']);
            foreach ($temparr as $key => $v) {
                $temparr2      = explode('$$$', $v);
                $advtitle[] = $temparr2[0];
            }
        ...
        unset($temparr2);
        $advcontent = array();
        if (!empty($vo['advcontent'])) {
            $temparr = explode('|||', $vo['advcontent']);
            foreach ($temparr as $key => $v) {
                $temparr2      = explode('$$$', $v);
        ...
        $adv = array();
        for($i=0;$i<5;$i++){
            $adv[$i]['title']=$advtitle[$i];
            $adv[$i]['content']=$advcontent[$i];
        }
        ...
        unset($temparr2);
        $apptitle = array();
        if (!empty($vo['apptitle'])) {
            $temparr = explode('|||', $vo['apptitle']);
            foreach ($temparr as $key => $v) {
                $temparr2      = explode('$$$', $v);
                $apptitle[] = $temparr2[0];
            }
        ...
        unset($temparr2);
        $appcontent = array();
        if (!empty($vo['appcontent'])) {
            $temparr = explode('|||', $vo['appcontent']);
            foreach ($temparr as $key => $v) {
                $temparr2      = explode('$$$', $v);
        ...
        //将典型应用的标题和内容整合为一个二维数组
        $application = array();
        $len = count($apptitle);
        for($i=0;$i<$len;$i++){
            $application[$i][0]=$apptitle[$i];
            $application[$i][1]=$appcontent[$i];
        }

        //将典型应用的图片转化为数组
        $apppicurls = array();
        if (!empty($vo['apppicurls'])) {
            $temparr = explode('|||', $vo['apppicurls']);
            foreach ($temparr as $key => $v) {
                $temparr2      = explode('$$$', $v);
                $apppicurls[] = array('url' => '' . $temparr2[0], 'alt' => '' . $temparr2[1]);
            }
        }
        ...
        $vo['adv'] = $adv;
        $vo['application'] = $application;
        $vo['apppicurls'] = $apppicurls;
    ...
    public function editPost()
    {
        ...
        if (false !== M('product')->save($data)) {
            ...
            //删除文件夹中的图片
            $imgDelUrls = array();
            $imgDelUrls     = isset($data['delImg']) ? $data['delImg'] : '';
            if (is_array($imgDelUrls)) {
                foreach ($imgDelUrls as $k => $v) {
                    $images[] = '.'.$v;
                    $url = substr($v,0,23);
                    $name = substr($v,23);
