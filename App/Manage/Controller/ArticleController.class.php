...
class ArticleController extends CommonContentController
{
    ...
    public function index()
    {
        ...
        $this->assign('type', $art[0]['catename'] . "列表");
	...
	public function loadNews(){
		...
		$count = 0;
		for($i; $i > 0;$i--){
			$news_url = $host.$url.'?Page='.$i;
			$sHtml = file_get_contents($news_url);
			$sHtml = iconv('gbk','utf-8',$sHtml);
			$sHtml = $meta.$sHtml;
			$oDom = new \Common\Lib\ParserDom($sHtml);
			$table = $oDom->find('table.text30', 1);
			//$tr = $table->find('td.Listsc');
			$tr = $table->find('tr');
			$tr = array_reverse($tr);
			// p($tr);exit;
			foreach($tr as $key=>$row){
				// 日期
				$date = $row->find('td',1)->getPlainText();
				$date = trim($date);
				$date = ltrim($date,'[');
				$date = rtrim($date,']');
				//p($date);
				$a = $row->find('td',0)->find('a.text20');
				// 标题
				/* $title = $a[0]->getPlainText();
				$title = trim($title);
				//p($title); */
				
				// 内容URL
				$content_url = $a[0]->getAttr('href');
				$content_url = $host . trim($content_url);
				// p($content_url);
				$scHtml = file_get_contents($content_url);
				$ocDom = new \Common\Lib\ParserDom($scHtml);
				// 标题
				$title = $ocDom->find('td.ListText',0)->getPlainText();
				$title = trim($title);
				// 内容 要把原网站的 Upimg 目录拷贝到新网站目录下
				$td = $ocDom->find('td.Text20');
				$content = $td[count($td)-1]->innerHtml();
				// 替换图片地址
				$pattern = '#(/Upimg/)#i';
				// <img src="/Upimg/200572710124450.jpg" border="0">
				$content = preg_replace($pattern,rtrim($host,'/').'$1',$content);
		...
		$end_time = time();
		$time = left_time($end_time - $start_time);
		print_r('采集结束，共采集 '.$count.' 条内容，耗时 '.$time);
	}
}
