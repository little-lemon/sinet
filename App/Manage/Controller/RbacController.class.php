...
class RbacController extends CommonController
{
    ...
    public function addRolePost()
    {
        ...
        if ($rid = M('role')->add($_POST)) {
            $access[] = array('role_id' => $rid, 'node_id' => 56, 'level' => 2);
            $access[] = array('role_id' => $rid, 'node_id' => 57, 'level' => 3);
            $access[] = array('role_id' => $rid, 'node_id' => 58, 'level' => 3);
            M('access')->addAll($access);
    ...
    public function access()
    {
		...
		$sql = "select b.catid from si_category as a join si_category_access b on a.pid = 0 and a.id = b.catid and b.roleid = $rid";
		$data = M()->query($sql);
		$accessStr = ',';
		foreach($data as $k => $v){
			$accessStr .= $v['catid'].',';
    ...
    public function accessPost()
    {
        ...
        //内容管理权限的插入
        $cateAccess = I('post.cateAccess');
        $cateAccessData = array();
        $index = 0;
        foreach($cateAccess as $k1 => $v1){
            $cateAccessData[$index]['catid']  = $v1;
            $cateAccessData[$index]['roleid'] = $rid;
            $cateAccessData[$index]['flag'] = 1;
            $cateAccessData[$index]['action'] = 'index';	
            $index ++;	
            $childData = M('category')->where("pid = $v1")->select();
       ...
       $ret2 = M('categoryAccess')->where("roleid =  $rid")->delete(); 
       $ret3 = M('categoryAccess')->addAll($cateAccessData);
        if ($ret && $ret2 && $ret3) {
            $this->success('配置成功', U('Rbac/role'));
        } else {
            $this->error('配置失败');
