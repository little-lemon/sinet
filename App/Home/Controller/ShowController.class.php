 ...
 * @Last Modified time: 2017-06-13 22:20:59
 */
...
//$cid来判断
    ...
    public function job()
    {
        $joblist = M('article') -> where("cid=58 and status=0")->field("title,content,publishtime")->select();
    ...
    {
        $id = I('get.id',0);
        $detail = M('culture')->where("id=$id")->select();
        $this->assign('pdetail',$detail[0]);
        $this->display();
    ...
    {
        $joblist = M('article') -> where("cid=99 and status=0")->field("title,content,publishtime")->select();
        $this->assign("joblist",$joblist);
        $this->display();
    }
