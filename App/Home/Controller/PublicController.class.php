 ...
 * @Last Modified time: 2017-06-14 20:26:47
 */
...
class PublicController extends HomeCommonController
{
    ...
    public function loginPost()
    {
        ...
        $uf = $user['id'].'.'.md5($password).'.'.get_random(6);//检测因子
        session('uid', $user['id']);
    ...
    public function _loginChk()
    {
        ...
        if (empty($user_id) || date('Y-m-d', $verifytime) != date('Y-m-d', time())) {
            ...
            if ($login_flag) {
                ...
                if (!$user || ($user['password'] != get_password_md5($user_flag[1], $user['encrypt']))) {
                    $login_flag = false;
                } else {
                    session('uid', $user['id']);
                }
...
str;
    ...
    public function sendActivate()
    {
        ...
        $actinfo = M('active')->where(array('userid' => $this->uid, 'expire' => array('gt', time())))->find();
        $data    = array();
        //有记录
        if ($actinfo) {
            $data['id']     = $actinfo['id'];
